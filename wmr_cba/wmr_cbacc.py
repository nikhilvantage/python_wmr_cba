"""
    ...
    CBACC - Class for talking to a WMR CBACC
...
class CBACC:
    """
    Class for talking to CBA Charge Controller.

    __init__(serial_number) (Constructor) - Open a CBACC.  If serial_number is
    provided, will attempt to open that specific CBACC.  If serial_number isn't
    provided, will attempt to open the first CBACC found.

    is_valid() - returns True if we are connected to a CBACC.

    close() - gracefully close the connection to the CBACC.

    @staticmethod scan() - Returns an array of found CBACC's serial numbers.
    ...
    is_running() - Returns True if the CBACC is performing a test and drawing
    current.

    is_power_limited() - Returns True if the CBACC is limiting current draw
    to prevent exceeding the max power limits of the unit.

    is_high_temp() - Returns True if the test was aborted because the CBACC 
    temperature was too high.
    """
    def __init__(self, serial_number=None, interface=None):
        debug("CBACC.__init__()")
    ...
    class __worker_thread(threading.Thread):
        ...
        and will stop charging or close relay.
        """
        ...
        def run(self):
            debug("CBACC.__worker_thread.run()")
            while self.__run:
                time.sleep(0.75)
        ...
        def stop(self):
            ...
            """
            debug("CBACC.__worker_thread.stop()")
            self.__run = False
            #end stop()
    ...
    def scan():
        ...
        i = 0
        while i < num:
            cba = CBACC(interface=MpOrLibUsb(i))
            i += 1
    ...
    def get_status_response(self, force_xmit=None, force_rcv=None):
        """
        Read the status message from the CBACC.
...
class MpOrLibUsb:
    ...
    @staticmethod get_device_count() - return number of CBACCs connceted.
    isValid() - returns True if we are connected to a CBACC device.
    ...
    def __init__(self, interface_number=0):
        ...
        if (interface_number < numMpusb):
            ...
            self.__handle_read = self.__usb_dev.MPUSBOpen(interface_number, "vid_2405&pid_0004", "\\MCHP_EP1", 1)
            self.__handle_write = self.__usb_dev.MPUSBOpen(interface_number, "vid_2405&pid_0004", "\\MCHP_EP1", 0)
            #end (inteface_number < numMpusb)
        else:
            # grab from pyusb/libusb
            interface_number = interface_number - numMpusb
            try:
                devs = usb.core.find(find_all=True, idVendor=0x2405, idProduct=0x0004)
                i = 0
                if devs:
                    for dev in devs:
    ...
    def test():
        ...
        ret = None
        try:
            dev = usb.core.find(idVendor=0x2405, idProduct=0x0004)
            if dev:
                dev.reset()
                dev = None
        except:
    ...
    def __get_device_count_Mpusb():
        ...
        num = MpUsbApi().MPUSBGetDeviceCount("vid_2405&pid_0004")
        return num
    ...
    def write(self, data, timeout_ms=0):
        """
        Write 'data' (bytearray) to CBACC, waits 'timeout_ms' for endpoint to be available for writing (0 is wait forever).
    ...
    def read(self, timeout_ms=0):
        """
        Read from CBACC, returns bytearray if success or None if nothing available.  Will wait 'timeout_ms', forever if set to 0.
        """
        if not self.is_valid():
            return None
        buf = None
...
def __test_cbacc():
    ...
    print("Opened CBACC, serial #" + str(cba.get_serial_number()))
